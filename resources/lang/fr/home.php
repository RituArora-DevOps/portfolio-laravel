<?php

return [
  'page_title' => 'Accueil',
  'name' => 'Dr. Ritu Arora',
  'tagline' => 'Développeuse Full Stack | Ingénieure DevOps | Chimiste computationnelle | Chercheuse scientifique | Apprenante perpétuelle',
  'intro' => 'Bienvenue sur mon portfolio ! Je conçois des solutions logicielles innovantes qui améliorent l’efficacité dans divers domaines.',
  'profile_alt' => 'Photo de profil de Dr. Ritu Arora',
  'projects_heading' => 'Projets en vedette',

  'car_rental' => [
    'title' => 'Système de gestion de location de voitures',
    'description' => 'Une plateforme web pour gérer les services de location de voitures, y compris les réservations, le suivi de la disponibilité et les dossiers clients.',
    'tech' => 'Technologies utilisées : HTML, CSS, JavaScript, NodeJS, MySQL',
  ],
  'daycare' => [
    'title' => 'Système de gestion de garderie',
    'description' => 'Un système complet pour gérer les dossiers des enfants, les éducateurs, les horaires et la communication avec les parents dans les centres de garde.',
    'tech' => 'Technologies utilisées : PHP Slim, MeekroDB, Twig, Bootstrap, JS, Apache',
  ],
  'portfolio' => [
    'title' => 'Site personnel de portfolio',
    'description' => 'Un portfolio personnel présentant mes compétences, projets et publications.',
    'tech' => 'Technologies utilisées : HTML, SASS, JavaScript, Bootstrap, Laravel, MySQL, Apache',
    'upcoming' => 'Amélioration à venir : Intégration Laravel pour un contenu dynamique et des fonctionnalités backend.',
  ],

  'source_code' => 'Code source',
  'ppt' => 'PPT',
  'view_live' => 'Voir en ligne',

  'previous' => 'Précédent',
  'next' => 'Suivant',

  'testimonials_heading' => 'Ce que les gens disent',
];
